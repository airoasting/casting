...
## 기본 설정 (No-Specify Defaults — v0.4.14 신설)

사용자가 출력 포맷·템플릿·분량·구조를 명시하지 않으면 메인 Claude는 다음 디폴트를 적용한다. 이 디폴트는 generic_case·매칭 케이스 모두에 적용되며, 케이스 정의에 명시된 포맷·구조가 있으면 케이스 정의가 우선한다.

### D1. 기본 출력 포맷 = HTML 슬라이드

사용자 입력에 "이메일·메모·보고서·MD·텍스트" 같은 비-슬라이드 신호가 없고, 동시에 케이스 정의가 Markdown을 강제하지 않으면 → **HTML 슬라이드 데크(`html_mode: slides` 모드)로 기본 출력**.

- generic_case에서 HTML 신호가 약해도 슬라이드를 디폴트로 잡는다.
- 케이스 매칭 결과가 Markdown 카테고리(이메일·사과문·LP 레터 등)면 케이스 정의를 따라 Markdown 유지.
- 사용자가 "MD로", "Markdown으로", "텍스트로", "메모로", "이메일로" 같이 명시하면 그쪽을 우선.

### D2. 기본 템플릿 우선순위 8종

`html_mode`가 `slides`·`landing`·`assisted`로 가는 모든 호출에서 Phase 2 §2-2 top-3 추출 시 다음 8종을 1차 풀로 본다. 입력 톤과 가장 맞는 1개를 top-1으로 채택하며, 8종 모두 부적합한 경우에만 35종 전체에서 다시 고른다.

| 순위 | 슬러그 | 한국어 이름 | color | formality | 적합 무드 |
|---|---|---|---|---|---|
| 1 | `dark-magazine` | 다크 매거진 | dark | formal | 매니페스토·비전·드라마틱·강렬 |
| 2 | `soft-classic` | 소프트 클래식 | light | formal | 우아·조용·에디토리얼·문학 |
| 3 | `retro-cassette` | 레트로 카세트 | light | casual | 레트로·카와이·따뜻·아날로그 |
| 4 | `retro-magazine` | 레트로 잡지 | light | casual | 인디 진·수공예·언더그라운드·웜 레트로 |
| 5 | `sunshine-yellow` | 선샤인 옐로 | light | formal | 전시·문화·따뜻한 햇살 |
| 6 | `warm-cream` | 따뜻한 크림 | light | neutral | 친밀·호스피탈리티·모임 |
| 7 | `soft-neumorph` | 소프트 뉴모피즘 | light | formal | 디지털·테크·정제·UX |
| 8 | `colorful-creative` | 알록달록 크리에이티브 | light | neutral | 크리에이티브·에이전시·유쾌 |

선정 절차:
1. 메인 Claude가 입력에서 톤·color·formality·핵심 인상을 추출.
2. 8종 각각에 적합성 점수 1~10 계산. 점수 7 이상이 1개라도 있으면 → 그 안에서 top-3 추출.
3. 모두 7 미만이면 → 35종 전체에서 다시 top-3 추출 (기존 Phase 2 §2-2 절차).
4. preferences 처리는 기존 Phase 2 §2-2 절차 그대로 (preferences가 8종 안에 있으면 1순위 유지).

이 룰은 "사용자가 별도 지정하지 않을 때 어떤 무드로 떨어질지 결정적으로 만든다." 모든 호출의 디자인 결과가 8종 안에 들어가 일관성이 확보된다.

### D3. 기본 슬라이드 수 = 5~10장

사용자가 "20장·짧게·1장·표지만" 같이 분량을 명시하지 않으면 → **5~10장**. 입력 복잡도에 따라 메인 Claude가 그 안에서 자기 판단으로 정한다.

- 메시지 1~2개면 5~6장
- 메시지 3~4개면 7~8장
- 메시지 5개면 9~10장

케이스 정의에 슬라이드 수가 명시되어 있으면 케이스 정의 우선.

### D4. 기본 스토리라인 = 3~5개의 조언·제언·메시지

사용자가 스토리 구조를 명시하지 않으면 → **3~5개의 핵심 메시지(조언·제언·인사이트)**를 한 축으로 잡는다. 각 메시지가 1~2개 슬라이드를 받는다.

표준 데크 구조:
1. **표지** (1장): 한 줄 약속.
2. **이슈 또는 맥락** (0~1장, 선택).
3. **메시지 1~5** (각 1~2장): 조언·제언·인사이트.
4. **클로징** (1장): 다음 액션 또는 한 줄 매니페스토.

핵심 메시지는 다음 형태 중 하나로 잡는다. BLACK이 입력에서 자동 추론한다.
- "X 하지 마라, Y 하라" 류의 반전형 조언
- "X 다시 보자" 류의 재정의형 제언
- "X면 Y다" 류의 단정형 인사이트
- "X에서 멈춰서 Y를 본다" 류의 관찰형 메시지

이 구조는 BLACK 프롬프트에 다음 블록으로 박힌다 (디폴트 적용 시):

```
[DEFAULT STORYLINE]
structure: 표지 + 메시지 3~5개 + 클로징
message_count: {3|4|5}  ← 입력 복잡도로 메인이 결정
slide_count: {5~10}     ← message_count + 표지·클로징
[/DEFAULT STORYLINE]
```

### D5. 디폴트 사용 시 사용자 알림 (1줄, 부담 0턴)

디폴트가 적용되면 Phase 7 deliver 메시지 직전에 다음 한 줄을 추가:

> 디폴트 적용: HTML 슬라이드 · {템플릿 이름} · {N}장 · 메시지 {M}개. 다른 무드/분량 원하시면 다음 호출에 명시해주세요.
...
5. **에러 폴백 (generic_case):** 모든 후보 신뢰도 < 0.5 → generic 5-Color 모드로 진행. 케이스 시드는 없지만 **워크플로우는 단축하지 않는다**.
   ...
   **5-1. 하이브리드 모드 (v0.4.16 신설 — top-1 신뢰도 0.40~0.50 회색지대 한정).**
   top-1 신뢰도가 0.40 이상 0.50 미만이면 페르소나 시스템 프롬프트는 generic 프로파일을 쓰되, **BLACK 톤 가이드와 GOLD 시나리오 힌트만 top-1 케이스 정의에서 차용**한다. 라우팅 fall-through로 모든 케이스 시드를 버리면 R1 톤이 매니페스토로 떨어지는 사례가 누적된다(예: c44 외부 강연·c14 블로그 톤이 generic으로 떨어지면서 R1 평균 9.2 시작). 하이브리드는 라우팅 라벨은 `generic_case`이지만 BLACK 프롬프트에 다음 블록을 추가로 박는다:
   ```
   [HYBRID TONE SEED FROM {top1_case_id}]
   black_tone_hint: {케이스 정의의 BLACK 페르소나 톤 한 단락}
   gold_scenario_hint: {케이스 정의의 GOLD 시나리오 한 단락}
   [/HYBRID TONE SEED]
   ```
   RGSB 4인은 generic 프로파일로 채점한다 (페르소나 시스템 프롬프트는 건드리지 않음). reasoning.md에 "하이브리드 모드 진입, top-1: {case_id} (신뢰도 {0.xx})" 한 줄 명시. 사용자 알림(1회): "케이스 라이브러리 매칭은 약하지만 {case_title}의 톤을 부분 차용합니다."
   ...
   **출력 포맷 추정 (v0.4.14 디폴트 갱신):**
   - 사용자 입력에 "이메일·메모·MD·텍스트·이력서·사과문·내부 공지" 같은 비-슬라이드 신호가 있으면 → 출력 포맷 `md`, Markdown 본문.
   - 사용자 입력에 "이미지·사진·로고·브랜드·시각·hero shot" 같은 신호가 있으면 → 출력 포맷 `md` (7단 이미지 프롬프트 세트, c74 톤 차용).
   - 사용자 입력에 "랜딩·랜딩페이지·single page·한 페이지·웹사이트" 신호가 있으면 → 출력 포맷 `html`, 빌더 모드 `landing`.
   - 사용자 입력에 "대시보드·일정표·인터랙티브·탭·필터" 신호가 있으면 → 출력 포맷 `html`, 빌더 모드 `assisted`.
   - **그 외 (디폴트) → 출력 포맷 `html`, 빌더 모드 `slides`.** 기본 설정 D1·D2·D3·D4를 그대로 적용.
...
3. **디폴트 풀 우선 평가 (v0.4.14 신설 — 기본 설정 D2 참조):**
   먼저 디폴트 8종(`dark-magazine`·`soft-classic`·`retro-cassette`·`retro-magazine`·`sunshine-yellow`·`warm-cream`·`soft-neumorph`·`colorful-creative`)에 적합성 점수 1~10 계산.
   - 8종 중 7점 이상이 1개라도 있으면 → 8종 안에서 top-3 추출하고 단계 4로.
   - 8종 모두 7점 미만이면 → 35종 전체에서 top-3 추출 (아래 단계 4).
   - 사용자가 디폴트 풀 밖 템플릿(예: 윈도우 95·네온 픽셀)을 명시 지목하면 → 디폴트 풀 우회, 35종 전체로 즉시.

4. 35종(또는 디폴트 풀 8종) 각각에 적합성 점수 1~10 계산 (도메인 키워드 일치 가중치 가장 큼).

5. top-3 후보 추출. 동점 시 formality 우선, 다음 color, 다음 slide_count.
...
6. preferences 처리:
   ...
   - **첫 호출 자동 저장 (v0.4.16 신설):** 해당 케이스·generic_case에 preferences 파일이 *없으면* Phase 7 deliver 직후 자동으로 `{output_dir 또는 ~/.claude/roasting}/preferences/{case_id|generic_case}.json`을 다음 포맷으로 저장한다:
     ```json
     {
       "template_id": "colorful-creative",
       "selected_at": "ISO-8601",
       "selection_reason": "위트 + 컨설턴트 청중에 직격, 디폴트 풀 8종 안 top-1"
     }
     ```
     다음 호출에서 동일 케이스/generic_case로 떨어지면 위 1순위 규칙에 따라 colorful-creative가 다시 1순위(단, top-3 평가는 그대로 수행). 일관성 ↑ + 사용자 학습 ↑. 사용자가 명시적으로 "다른 거"를 요청하면 갱신.

7. 1순위 + 2·3순위 + 각각 선택 사유 한 줄을 reasoning.md의 'Slide Template Selection' 절에 기록. 이 절은 모든 HTML 산출물에 반드시 존재. **디폴트 풀 8종 안에서 선택했는지 35종 전체에서 선택했는지도 표기**.

8. 사용자에게 1줄 알림(부담 0턴):
   "디자인 템플릿: [dark-magazine] 선택 (디폴트 풀 8종 top-3: dark-magazine · soft-classic · sunshine-yellow). 다른 선호 있으시면 알려주세요."

9. 사용자가 명시적으로 "다른 거"를 요청하면 2순위로 교체, preferences 갱신.

10. 결정 직후 BLACK 호출 시 슬라이드 템플릿 URL을 BLACK 프롬프트에 명시. BLACK은 그 URL을 WebFetch로 받아 베이스로 사용 (agents/roasting-black.md "HTML 산출물 베이스 룰" §1 강제).
```
...
4. **추상 주제 감지 + 도메인 어휘 화이트리스트 (v0.4.16 신설 — generic_case·하이브리드 한정).**
   - **트리거:** 사용자 입력이 generic_case 또는 하이브리드 모드로 분류되었고, 동시에 추상 주제(AI 시대·미래·리더십·전략·트렌드·조언·인사이트 같은 어휘를 포함하지만 구체 도메인 사실은 없음)인 경우 → `abstract_topic_detected = True`. 메인 Claude가 자체 판정.
   - **이유:** 추상 주제 + 케이스 시드 부재 = R1 SILVER drag 패턴. SILVER가 "현장 어휘가 비었다, 바깥에서 본 매니페스토 같다"로 9.0 아래로 깎는다. R1 평균 8.7~9.2 발생, R2~R3 추가 라운드 강제. 사전에 도메인 어휘를 BLACK에 시딩해 R1 SILVER를 9.2+에서 시작시킨다.
   - **도메인 어휘 화이트리스트 (BLACK 프롬프트에 "본문에 자연스럽게 최소 5개 박을 것"으로 박음):**
     - **컨설팅·전략:** Day-1 Hypothesis · So-what · Pyramid Principle · MECE · 가설 트리 · 인터뷰 가이드 · Final Report · Appendix · Synthesize · Steering Committee · 디스가이즈
     - **마케팅·그로스:** CAC · LTV · 퍼널 · 코호트 · 어트리뷰션 · 리텐션 · ARPU · 노쇼 · 활성화 · 인입
     - **재무·IR:** working capital · covenant · DSO · DPO · EBITDA · run rate · 가이던스 · MD&A · 분기 사업보고서 · 발행 단가
     - **HR·조직:** 평가 사이클 · 1on1 · 캘리브레이션 · 9-box · 백필 · 시그마 · 리스킬링 · 직무 기술서
     - **VC·스타트업:** 텀시트 · 청산 우선권 · 안티딜루션 · 라운드 사이즈 · IC 메모 · 벨류에이션 · burn · 런웨이 · pro rata
     - **AI·테크:** 토큰 · 파인튜닝 · RAG · 에이전트 · 컨텍스트 윈도우 · 추론 비용 · MoE · 멀티모달 · evals
     - **법무·규제:** 약식 명령 · 사실관계 · 청구 취지 · 처분 사유 · 행정심판 · 가처분 · 임의 명령 · 변론 종결
     - 사용자 입력에서 도메인을 메인 Claude가 추론하지 못하면, 최상위 3개 후보를 BLACK 프롬프트에 모두 첨부하고 BLACK이 본문 톤에 맞춰 선택하도록 위임.
   - **결과 저장:** `pre-processing.json`에 `abstract_topic_detected`, `inferred_domain`, `vocab_whitelist` 필드 추가.
   - **금지:** 화이트리스트 어휘를 기계적으로 5개 채우기만 하는 BLACK. 본문 흐름에서 자연스럽게 박혀야 함. BLACK 프롬프트에 "위트 톤이라면 잡담처럼 끼우고, 단정 톤이라면 정의처럼 박는다" 명시.

5. **Pre-Processing Pack 저장.**
   - 위 1~4 결과를 `{session_dir}/pre-processing.json`에 저장. BLACK·RGSB 모두 이 파일을 참조한다.
   - 포맷: `{"dates_resolved": {...}, "param_defaults": {...}, "primary_sources": [...], "abstract_topic_detected": bool, "inferred_domain": "...", "vocab_whitelist": [...]}`.
...
6. **BLACK 프롬프트 첨부.**
   - Phase 3 BLACK dispatch 시 Pre-Processing Pack을 다음 블록으로 user prompt 머리에 박는다:
     ...
     abstract_topic_detected: true/false
     inferred_domain: 컨설팅·전략  ← 추상 주제일 때만
     vocab_whitelist: [Day-1 Hypothesis, So-what, Pyramid Principle, ...]  ← 본문에 자연스럽게 최소 5개 박을 것
     [/PRE-PROCESSING PACK]
     ```
...
message_count: {3|4|5}  ← 입력 복잡도로 메인이 결정
slide_count: {5~10}     ← message_count + 표지·클로징
message_form: 조언·제언·인사이트 중 입력 톤에 맞는 형태
  - 반전형: "X 하지 마라, Y 하라"
  - 재정의형: "X 다시 보자"
  - 단정형: "X면 Y다"
  - 관찰형: "X에서 멈춰서 Y를 본다"
...
사용자가 분량·구조를 명시했거나 케이스 정의가 자체 구조를 강제하면 이 블록은 박지 않는다.
...
목적: BLACK 산출물의 10종 안티패턴 검출 → 발견 시 BLACK 재작성 (라운드 카운트 안 깎임). 3-strikes 보호.

**검출 룰 10종:** `hallucinated-number` · `legal-risk-term` · `missing-gold-hook` · `tone-mismatch` · `vague-cta` · `unsourced-fact` · `fake-image-url` · `internal-contradiction` · `slide-template-violation` · `english-verb-direct`. 각 룰의 `applies_to_categories`로 케이스 카테고리별 적용 여부 결정.

**`english-verb-direct` 룰 (v0.4.13 신설):** 영어 일반 동사를 한국어로 그대로 옮긴 직역체 검출. break → "깨진다" (직무·모델·시스템) / kill → "죽인다" (가설·아이디어·옵션) / force·make → "시킨다" (AI에게 X를 시키다). v0.4.12 회의실 jargon이 영어 imperative 어휘 직역(drive → 박다)을 잡았다면, 본 룰은 영어 일반 동사의 의미 연결 직역(kill the hypothesis → 가설을 죽인다)을 잡는다. 메인 Claude가 Phase 4에서 3패턴 grep 자동 실행. 자연 한국어 대체 풀: 흔들다·접다·다듬다·풀다·받다·두다. 상세는 `references/anti-patterns/english-verb-direct.md`.
...
|---|---|
| 평균 ≥ 9.5 | Phase 7 |
| 9.4 ≤ 평균 < 9.5 + round_count < 4 + Δ(R_n − R_{n-1}) < 0.1 | **수렴 saturation 컨펌 (v0.4.16 신설).** 사용자에게 1턴 confirm — "(1) 한 라운드 더 돌려 9.5+ 시도 (2) 9.4x 그대로 출력 (3) 정밀 모드 한국어 폴리시까지 (4) 중단". round_count 4 미만이고 1라운드 개선폭이 0.1 이상이면 자동 진행. |
...
#### 6.0 BLACK 라운드 간 컨텍스트 유지 (v0.4.16 신설)

라운드 사이 BLACK 재호출은 **새 Agent dispatch가 아닌 SendMessage 경로**를 우선 시도한다. R1에서 띄운 BLACK 서브에이전트의 `agentId`를 `{session_dir}/meta.json`에 저장하고, R2~R4에서는 같은 agentId로 SendMessage. 컨텍스트 유지 + `agents/roasting-black.md` 재로드 비용 절감 + 라운드 일관성 ↑.

- **Step 1:** Phase 3 R1 BLACK dispatch 직후 반환된 agentId를 `meta.json`에 `black_agent_id`로 기록.
- **Step 2:** Phase 6에서 라운드 backedge 진입 시 `ToolSearch`로 `SendMessage` 도구 스키마를 사전 로드 (deferred 도구 → 호출 직전 검색 필수).
- **Step 3:** `SendMessage(to=black_agent_id, content={rgsb_comments, prev_draft_path, round_n})`로 재호출. 응답 파일 경로는 다른 R1과 같은 위치(`{session_dir}/round-{n}/black-draft.{html|md}`).
- **에러 폴백:** SendMessage 실패(`AgentNotFound`·`Stale`) → 새 Agent dispatch로 폴백. meta.json의 agentId 갱신.
...
3. **L3 양성 hit 컨텍스트 검증 (v0.4.16 신설 — false positive 차단).** L3 카운트가 1 이상이면, 각 hit에 대해 자동으로 30자 컨텍스트 grep 수행 (`grep -oE ".{30}패턴.{30}"`). 메인 Claude가 hit 별로 다음 룰로 분류:
   - **정상 한국어 용법** → L3 카운트에서 제외. 예: "에 대한" 앞에 의존명사가 오는 "것에 대한·지에 대한·곳에 대한" / "에 있어서"가 강조 부사 뒤 오는 "특히 X에 있어서는 Y" / "할 수 있습니다"가 가능성 평가 문맥("리스크가 발생할 수 있습니다")인 경우.
   - **번역투 양성** → L3 카운트 유지. 예: "X에 대한 분석" / "X에 대한 접근" 같이 영어 "analysis of X / approach to X" 직역 패턴.
   - 분류 결과를 `korean-polish.md`에 "L3 hit 분석" 절로 한 줄씩 기록 (hit · 컨텍스트 · 분류 · 사유).
4. AI 티 점수 산출 (L3는 컨텍스트 검증 후 조정값 사용): `점수 = 10 − (L3_adjusted × 2.0) − (L2 × 0.5) − (L1 × 0.2)`
5. 등급 판정:
   - **A**: 점수 ≥ 9.0, L3_adjusted = 0 → 사람 글로 통과 ✅
   - **B**: 점수 8.0~8.9, L3_adjusted = 0, L2 ≤ 4 → 합격 ✅
   - **C**: L3_adjusted 1~2개 또는 점수 7.0~7.9 → 사용자 컨펌 (정밀 모드 권고)
   - **D**: L3_adjusted ≥ 3개 또는 점수 < 7.0 → 사용자 컨펌 (사람 검수 권고)
6. 6항 자체검증 (의미 동등·변경률·말투·장르·등급·em dash)
7. 결과를 `final/korean-polish.md`에 별도 파일로 보고
...
4. **표준 완료 푸터 (모든 호출에 강제):**
   ...
   - 카카오톡: 📱 발송됨
   - 디폴트 적용: HTML 슬라이드 · dark-magazine · 7장 · 메시지 4개 (사용자 미지정 시에만 표시)
...
| `references/anti-patterns/*.md` | 10종 검출 룰 (`hallucinated-number`, `legal-risk-term`, `missing-gold-hook`, `tone-mismatch`, `vague-cta`, `unsourced-fact`, `fake-image-url`, `internal-contradiction`, `slide-template-violation`, `english-verb-direct`) + BLACK 재작성 지시 | Phase 4 (적용 안티패턴만) |
...
`/roasting xxxxx` (또는 자연어) → 68 케이스 자동 라우팅 → BLACK 1명 + RGSB 4인 5-Color → 9.5 합격선 + 4라운드 → 산출물 4종. 사용자 미지정 시 디폴트(HTML 슬라이드 · 8종 우선 풀 · 5~10장 · 메시지 3~5개)로 자동 떨어짐.
