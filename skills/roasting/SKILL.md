...
`html_mode`가 `slides`·`landing`·`assisted`로 가는 모든 호출에서 Phase 2 §2-2 top-3 추출 시 다음 7종을 1차 풀로 본다. 입력 톤과 가장 맞는 1개를 top-1으로 채택하며, 7종 모두 부적합한 경우에만 35종 전체에서 다시 고른다.
...
| 4 | `sunshine-yellow` | 선샤인 옐로 | light | formal | 전시·문화·따뜻한 햇살 |
| 5 | `warm-cream` | 따뜻한 크림 | light | neutral | 친밀·호스피탈리티·모임 |
| 6 | `soft-neumorph` | 소프트 뉴모피즘 | light | formal | 디지털·테크·정제·UX |
| 7 | `colorful-creative` | 알록달록 크리에이티브 | light | neutral | 크리에이티브·에이전시·유쾌 |

선정 절차:
1. 메인 Claude가 입력에서 톤·color·formality·핵심 인상을 추출.
2. 7종 각각에 적합성 점수 1~10 계산. 점수 7 이상이 1개라도 있으면 → 그 안에서 top-3 추출.
3. 모두 7 미만이면 → 35종 전체에서 다시 top-3 추출 (기존 Phase 2 §2-2 절차).
4. preferences 처리는 기존 Phase 2 §2-2 절차 그대로 (preferences가 7종 안에 있으면 1순위 유지).

이 룰은 "사용자가 별도 지정하지 않을 때 어떤 무드로 떨어질지 결정적으로 만든다." 모든 호출의 디자인 결과가 7종 안에 들어가 일관성이 확보된다.
...
3. **디폴트 풀 우선 평가 (v0.4.14 신설 — 기본 설정 D2 참조):**
   먼저 디폴트 7종(`dark-magazine`·`soft-classic`·`retro-cassette`·`sunshine-yellow`·`warm-cream`·`soft-neumorph`·`colorful-creative`)에 적합성 점수 1~10 계산.
   - 7종 중 7점 이상이 1개라도 있으면 → 7종 안에서 top-3 추출하고 단계 4로.
   - 7종 모두 7점 미만이면 → 35종 전체에서 top-3 추출 (아래 단계 4).
   - 사용자가 디폴트 풀 밖 템플릿(예: 윈도우 95·네온 픽셀)을 명시 지목하면 → 디폴트 풀 우회, 35종 전체로 즉시.

4. 35종(또는 디폴트 풀 7종) 각각에 적합성 점수 1~10 계산 (도메인 키워드 일치 가중치 가장 큼).
...
7. 1순위 + 2·3순위 + 각각 선택 사유 한 줄을 reasoning.md의 'Slide Template Selection' 절에 기록. 이 절은 모든 HTML 산출물에 반드시 존재. **디폴트 풀 7종 안에서 선택했는지 35종 전체에서 선택했는지도 표기**.

8. 사용자에게 1줄 알림(부담 0턴):
   "디자인 템플릿: [dark-magazine] 선택 (디폴트 풀 7종 top-3: dark-magazine · soft-classic · sunshine-yellow). 다른 선호 있으시면 알려주세요."
...
`/roasting xxxxx` (또는 자연어) → 68 케이스 자동 라우팅 → BLACK 1명 + RGSB 4인 5-Color → 9.5 합격선 + 4라운드 → 산출물 4종. 사용자 미지정 시 디폴트(HTML 슬라이드 · 7종 우선 풀 · 5~10장 · 메시지 3~5개)로 자동 떨어짐.
