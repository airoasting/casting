...
하고 싶은 일을 명령하면 오케스트레이터 팀장이 50명 가운데 필요한 에이전트를 골라 팀을 꾸리고, 그 팀을 **실제로 실행해** 검토를 통과한 결과물까지 만들어 냅니다.

**라이브 데모**: [50agents.airoasting.com](https://50agents.airoasting.com). 50명 카탈로그와 팀 빌더를 브라우저에서 바로 봅니다.

## 왜 만들었나

보고서 한 편에도 여러 사람의 손이 갑니다. 자료를 모으는 사람, 숫자를 읽는 사람, 글로 옮기는 사람, 틀린 곳을 잡아내는 사람이 각각 다릅니다. 같은 일을 AI에 맡기면 보통 한 모델이 혼자 다 합니다. 자기가 쓴 글을 스스로 검토하면 관대해지기 마련입니다.
...
안에는 역할 프롬프트 50개와 미리 짜 둔 팀 구성 28개가 들어 있습니다.

## 작동 방식

| 단계 | 수행 내용 |
|---|---|
| ① 목적 파악 | 산출물의 종류와 대상, 이를 채울 자료의 유무를 확인합니다. 자료가 없으면 먼저 요청합니다. |
| ② 팀 설계 | 기존 팀 구성을 선정하거나 팀원을 새로 조합합니다. 동일한 입력에는 동일한 팀이 편성됩니다. |
| ③ 팀 구조 제시 | 담당과 순서, 단계별 산출물을 실행 전에 제시합니다. |
| ④ 실행 | 팀원을 각각 독립 에이전트로 실행합니다. 상호 의존이 없는 단계는 병렬로 처리합니다. |
| ⑤ 검토 | 검토자가 결함부터 확인합니다. 9.5에 미달하면 해당 단계로 1회 반려합니다. |

구성은 세 층위입니다.

- **에이전트 팀원 50명**. 단일 역할을 수행하는 시스템 프롬프트입니다.
- **팀 구성 28종**. 팀원을 핸드오프로 연결해 미리 편성해 둔 팀입니다. 리서치 보고서, 시장 분석, 재무 검토, 발표자료, 전략 수립, 회의 정리 등이 있습니다.
- **라우터**. 목적을 해석해 기존 팀 구성을 선정하거나 팀원을 새로 조합합니다. 이 스킬의 핵심입니다.

**Claude Code 환경에서 가장 온전히 작동합니다.** 팀원이 각각 독립 에이전트로 실행되고 검토자도 별도로 기동되므로, 9.5 게이트가 가장 엄격하게 적용됩니다. Codex CLI에도 동일한 파일을 설치해 사용할 수 있습니다.
...
새 세션을 시작하면 Codex가 SKILL.md의 설명을 읽고 필요할 때 불러옵니다. GPT-5.6부터 Codex는 서브에이전트를 지원하므로 팀원을 병렬로 띄울 수 있고, 파일시스템이 있어 `_workspace/` 배치도 그대로 동작합니다. 팀원 50명과 팀 구성 28개, 9.5 게이트는 같습니다.
...
├── harnesses.md           # 28개 팀 구성 + 라우터 결정 사다리 + 토글
...
└── assets/               # prompts.js(프롬프트 정본) · router.js(팀 구성 정본) · agents · logos
```

## 변경 이력

**v1.2 (2026-08-15)**
역할 번호를 한 곳에서만 관리합니다. `scripts/sync_refs.py`가 `references/`를 만듭니다. 팀원 이미지 고지와 외부 에셋 출처(`NOTICE`)를 넣었습니다.
