...
보고서 한 편에도 여러 사람의 손이 갑니다. 자료를 모으는 사람, 숫자를 보는 사람, 글로 쓰는 사람, 틀린 데를 잡는 사람이 다릅니다. AI에게 이 일을 시키면 보통 한 모델이 혼자 다 합니다. 자기가 쓴 글을 자기가 검토하니 틀린 자리가 그대로 남습니다.

Casting은 그 일을 나눕니다. 팀원마다 다른 에이전트가 붙고, 검토자는 산출물을 만들지 않은 사람이 맡습니다. 사용자는 무엇을 만들지만 말하면 됩니다. 누가 어떤 순서로 할지는 팀장이 정하고 끝까지 끌고 갑니다.

AI ROASTING이 5색 다관점 채점으로 검증한 역할 프롬프트 50개와 팀 레시피 28개가 들어 있습니다.
...
| ② 팀 설계 | 레시피를 고르거나 부품을 새로 조합합니다. 같은 입력이면 같은 팀이 나옵니다. |
| ③ 팀 구조 제시 | 누가 어떤 순서로 무엇을 낼지 먼저 보여 줍니다. |
| ④ 실행 | 팀원을 각각 독립 에이전트로 띄웁니다. 서로 상관없는 단계는 동시에 돌립니다. |
| ⑤ 검토 | 검토자가 결함부터 찾습니다. 9.5를 못 넘기면 그 단계로 한 번 되돌립니다. |
...
**Claude Code에서 가장 잘 돕니다.** 팀원이 각각 독립 에이전트로 돌고 검토자도 따로 뜹니다. 그래서 9.5 게이트가 제일 세게 걸립니다. ChatGPT에서도 쓸 수 있습니다. 이때는 한 모델이 역할을 순서대로 맡습니다.
...
한 줄 수정, 단순 검색, 계산처럼 혼자 끝나는 일에는 쓰지 않습니다. 여러 사람의 손이 필요한 일에만 씁니다.
...
정확성·근거, 목적·완결성, 구조·형식, 실행력, 언어·톤 다섯 축으로 채점합니다. 한 축이라도 9.0 아래면 평균과 상관없이 떨어집니다. 자료가 없어서 못 채운 자리는 완성본처럼 덮지 않고 무엇이 더 필요한지 적어 돌려줍니다.

실제 테스트에서 게이트는 첫 산출물을 6.5로 떨어뜨리고 다시 만들라고 했습니다.
